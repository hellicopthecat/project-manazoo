...
/**
 * 데이터베이스 연결을 관리하는 클래스 DatabaseConfigLoader에서 설정 정보를 받아 실제 MySQL 연결을 처리합니다.
 */
	...
	public static Connection getConnection() throws SQLException {
		try {
			...
			if (configLoader.isShowSql()) {
				System.out.println(MenuUtil.DEFAULT_PREFIX + "데이터베이스 연결 성공: " + configLoader.getEnvironment() + " 환경");
			}

			return connection;

		} catch (ClassNotFoundException e) {
			throw new SQLException("MySQL JDBC 드라이버를 찾을 수 없습니다. " + "mysql-connector-java JAR 파일이 클래스패스에 있는지 확인해주세요.",
					e);
		}
	}
	...
	public static void closeConnection(Connection connection) {
		if (connection != null) {
			try {
				connection.close();
				if (configLoader.isShowSql()) {
					System.out.println(MenuUtil.DEFAULT_PREFIX + "데이터베이스 연결 종료됨");
				}
			} catch (SQLException e) {
