...
public class EnclosureManager {
	...
	private void printEnclosureInfo(String title, Enclosure enclosure) {
		String[] headers = { "Enclosure ID", "Name", "Size(m2)", "Temp(C)", "Location", "Environment", "Inhabitants",
				"Caretakers" };
		String[] values = { enclosure.getId(), enclosure.getName(), String.format("%.1f", enclosure.getAreaSize()),
				String.format("%.1f", enclosure.getTemperature()), enclosure.getLocationType().toString(),
				enclosure.getEnvironmentType().toString(), String.valueOf(enclosure.getInhabitantCount()), // 거주 동물 수
	...
	private void viewAllEnclosures() {
		...
		for (int i = 0; i < allEnclosures.size(); i++) {
			...
			data[i] = new String[] { enclosure.getId(), enclosure.getName(),
					String.format("%.1f", enclosure.getAreaSize()), String.format("%.1f", enclosure.getTemperature()),
					enclosure.getLocationType().toString(), enclosure.getEnvironmentType().toString(),
					String.valueOf(inhabitantCount), String.valueOf(caretakerCount) };
		}
	...
	private void displayEnclosureInhabitants(Enclosure enclosure) {
		...
		for (Map.Entry<String, Animal> entry : inhabitants.entrySet()) {
			...
			data[index] = new String[] { truncateString(animalId, 15), truncateString(animal.getName(), 15),
					truncateString(animal.getSpecies(), 15), truncateString(String.valueOf(animal.getAge()), 15),
					truncateString(animal.getGender(), 15), truncateString(animal.getHealthStatus(), 15) };
			index++;
		}

		String title = String.format("%s (%s) 거주 동물 목록", truncateString(enclosure.getName(), 15), enclosure.getId());
	...
	private void displayEnclosureCaretakers(Enclosure enclosure) {
		...
		for (Map.Entry<String, ZooKeeper> entry : caretakers.entrySet()) {
			...
			data[index] = new String[] { truncateString(keeperId, 15), truncateString(keeper.getName(), 15),
					truncateString(keeper.getDepartment().toString(), 15),
					truncateString(keeper.getRank().toString(), 15), keeper.isCanHandleDangerAnimal() ? "가능" : "불가능" };
			index++;
		}

		String title = String.format("%s (%s) 배정된 사육사 목록", truncateString(enclosure.getName(), 15), enclosure.getId());
		...
		if (repository.count() == 0) {
			...
			if (foundEnclosure.isPresent()) {
				...
				if (confirmed) {
					...
					if (deleted) {
						System.out
								.println("사육장 '" + enclosure.getName() + "' [" + enclosureId + "]이(가) 성공적으로 삭제되었습니다.");
					} else {
	...
	private void displayWorkingAnimalsTable(Map<String, Animal> workingAnimals) {
		...
		for (Map.Entry<String, Animal> entry : workingAnimals.entrySet()) {
			...
			data[index] = new String[] { animalId, animal.getName(), animal.getSpecies(),
					String.valueOf(animal.getAge()), "대기중" };
			index++;
		}
	...
	private boolean simulateAnimalAdmission(String enclosureId, String animalId, Map<String, Animal> workingAnimals) {
		...
		// 4. 시뮬레이션 성공 - 로그 출력
		System.out.println(
				String.format("시뮬레이션 성공: %s를 %s 사육장에 배치할 수 있습니다.", animal.getName(), enclosureOpt.get().getName()));
	...
	private void displayAdmissionResult(String enclosureId) {
		...
		String title = String.format("%s (%s) 현재 거주 동물 현황", truncateString(enclosure.getName(), 15), enclosureId);
	...
	private void manageKeeperAssignment() {
		...
		List<ZooKeeper> availableKeepers = ZooKeeperManager.getInstance().getWorkingKeepers();
	...
	private boolean hasRequiredDataForKeeperAssignment() {
		...
		boolean hasWorkingKeepers = ZooKeeperManager.getInstance().hasWorkingKeepers();
	...
	private void displayWorkingKeepersTable(List<ZooKeeper> availableKeepers) {
		...
		for (int i = 0; i < availableKeepers.size(); i++) {
			...
			data[i] = new String[] { truncateString(keeperId, 15), truncateString(keeper.getName(), 15),
					truncateString(keeper.getDepartment().toString(), 15),
					truncateString(keeper.getRank().toString(), 15), keeper.isCanHandleDangerAnimal() ? "가능" : "불가능" };
		}
	...
	private boolean executeKeeperAssignment(String enclosureId, String keeperId) {
		try {
			ZooKeeper keeper = ZooKeeperManager.getInstance().getRepository().getZooKeeperById(keeperId);
			if (keeper == null) {
				return false;
			}
	...
	private void displayKeeperAssignmentResult(String enclosureId) {
		...
		String title = String.format("%s (%s) 배정된 사육사 현황", truncateString(enclosure.getName(), 15), enclosureId);
