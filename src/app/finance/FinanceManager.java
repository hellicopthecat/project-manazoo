...
public final class FinanceManager {
	...
	private FinanceManager() {
		incomes.add(new IncomeExpend(IdGeneratorUtil.generateId(), 3000000l, "설명", IncomeExpendType.INCOME,
				EventType.AQUASHOW));
		expends.add(new IncomeExpend(IdGeneratorUtil.generateId(), 4000000l, "설명", IncomeExpendType.EXPEND,
				EventType.AQUASHOW));
		expends.add(new IncomeExpend(IdGeneratorUtil.generateId(), 2000000l, "설명", IncomeExpendType.EXPEND,
				EventType.AQUASHOW));
		incomes.add(new IncomeExpend(IdGeneratorUtil.generateId(), 4000000l, "설명", IncomeExpendType.INCOME,
				EventType.AQUASHOW));
		expends.add(new IncomeExpend(IdGeneratorUtil.generateId(), 1000000l, "설명", IncomeExpendType.EXPEND,
				EventType.AQUASHOW));
		expends.add(new IncomeExpend(IdGeneratorUtil.generateId(), 3000000l, "설명", IncomeExpendType.EXPEND,
				EventType.AQUASHOW));
	...
	private void editAssetData() {
		...
		while (run.get()) {
			...
			switch (index) {
			case 1 -> {
				System.out.println("추가되는 금액이나 감소하는 금액을 입력하세요.");
				Long money = Long.parseLong(InputUtil.getStringInput());
				setCapital(money);
				System.out.println("금액이 수정되었습니다. 현재 회사의 자산은 " + capital + " 원 입니다.");
			}
			case 0 -> goBack(run);
			default -> wrongIndex();
			}
