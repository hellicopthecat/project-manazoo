...
public class ZooKeeperManager {
	...
	private ZooKeeperManager() {
		repository.createZooKeeper(IdGeneratorUtil.generateId(), "루피", 20, 1, 2, 1, 1, 1, 1, "");
		repository.createZooKeeper(IdGeneratorUtil.generateId(), "크롱", 22, 2, 1, 2, 1, 2, 1, "자격증, 자격증2");
		repository.createZooKeeper(IdGeneratorUtil.generateId(), "뽀로로", 32, 1, 1, 3, 1, 7, 2, "");
		repository.createZooKeeper(IdGeneratorUtil.generateId(), "그롬마쉬 헬스크림", 40, 1, 1, 4, 1, 12, 2, "자격증, 자격증2, 자격증3");
		repository.createZooKeeper(IdGeneratorUtil.generateId(), "블랙헨드", 44, 1, 1, 5, 2, 14, 1, "자격증 ");
		repository.createZooKeeper(IdGeneratorUtil.generateId(), "마운트킹", 50, 1, 1, 6, 1, 25, 1,
				"자격증, 자격증2, 자격증3, 자격증4");
	}
	...
	public void handleZookeeperManagement() {
		...
		while (run.get()) {
			String[] menu = { "사육사 등록", "사육사 찾기", "사육사 수정", "사육사 삭제" };
			String[] s_menu = { "뒤로가기" };
			MenuUtil.generateMenuWithTextTitle("메뉴를 고르세요.", menu, s_menu);
	...
	private void getZooKeeperByDepartment() {
		...
		UIUtil.printSeparator('━');
		System.out.println(repository.getZooKeeperByDepartment(departmentIndex));
		UIUtil.printSeparator('━');
	}
