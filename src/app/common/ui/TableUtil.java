...
/**
 * 120자 콘솔 너비에 최적화된 표 형식 출력 유틸리티 클래스입니다. Windows CMD 환경에서 안정적으로 동작하도록 설계되었습니다.
 * 
 * <h3>사용 예시:</h3>
 * 
 * <pre>
 ...
 * String[][] data = { { "Apple", "$1.50", "100" }, { "Banana", "$0.80", "150" }, { "Orange", "$2.00", "80" } };
 * TableUtil.printTable("Product List", headers, data);
 * 
 * // 리스트를 이용한 다중 행 표 출력
 * List&lt;String[]&gt; dataList = Arrays.asList(new String[] { "Item1", "Value1" }, new String[] { "Item2", "Value2" });
...
public final class TableUtil {
	...
	/**
	 * 문자열의 실제 콘솔 표시 폭을 계산합니다. 한글, 중국어, 일본어 등은 2칸, 영문/숫자는 1칸으로 계산합니다.
	 * 
	 * @param text 측정할 텍스트
	 * @return 실제 콘솔 표시 폭
	 */
	...
	/**
	 * 전각 문자인지 판단합니다. CMD 기본 환경 호환성을 고려하여 한글과 CJK 문자만 처리합니다.
	 * 
	 * @param c 판단할 문자
	 * @return 전각 문자 여부
	 */
	...
	/**
	 * 텍스트를 고정 너비(15자)에 맞춥니다. 실제 콘솔 표시 폭을 고려하여 나머지는 우측에 공백으로 채웁니다.
	 * 
	 * @param text  원본 텍스트
